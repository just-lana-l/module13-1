...
# выданный на n_years лет под i_year_percent% годовых,
...
# A = KS
# 
# K = i_year_percent(1 + i_year_percent) ** n_years / (1 + i_year_percent) ** n_years - 1
# 
# Пример:
# 
...
#i_year_percent = float(input('Введите проценты по кредиту: '))
#n_years = int(input('Введите количество лет: '))
...
# A = KS - Аннуитетный платеж. Тело + проценты
# 
# K = i_year_percent(1 + i_year_percent) ** n_years / (1 + i_year_percent) ** n_years - 1
#

print('Проценты за первый год')
percent = (S*n_years)/100
print(percent)
print(' ')

K = ((i_year_percent*(1 + i_year_percent)**n_years) / ((1 + i_year_percent)**n_years - 1))
print('Какой-то коэффициент', K)
...
print('Остаток на начало периода', S)
print('Выплачено процентов', percent)
print('Выплачено Тело кредита', body)
